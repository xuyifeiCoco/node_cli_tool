...
async function updateData(config) {
    let { localFilePath, webDir, commond, projectName, prefentSh, afterSh } = config;
    try {
        if (prefentSh) {
...
async function connectSSH(config) {
    const { host, port, username, password, privateKey, passphrase, webDir } = config;
    const sshConfig = {
        host,
        port,
        username,
